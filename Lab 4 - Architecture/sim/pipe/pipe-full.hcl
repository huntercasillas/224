...
# Describe how and why you modified your code:
# I did most of my optimization in ncopy.ys (more info in that file)
# All I did in pipe-full.hcl was implement the iaddq instruction.
# I completed the logic by adding the iaddq instruction to instr_valid, 
# need_regids, need_valC, d_srcB, d_dstE, aluA, aluB, and set_cc.
#
#                       Description:
#       1.iaddq
...
# iaddq instruction is used to add a value to a register. 
# The register will store the final result. 
# Fetch needs to get the iaddq instruction and make sure that it's valid.
# Decode reads the data associated with iaddq. 
# Execute will take the data and insert the known value into the register 
# (adding if needed). The condition codes are also set here. 
# The iaddq instruction functions similarly to iopq and irmovq.

# The notation icode:ifun indicates the two components of the instruction byte.
# The notation rA:rB indicates the two components of the register specifier byte.
# The notation M1[x] indicates accessing (reading or writing) 1 byte at memory location x.
# The notation M8[x] indicates accessing 8 bytes at memory location x. 
